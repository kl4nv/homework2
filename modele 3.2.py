def send_email(message, recipient, *, sender='uneversity.help@gmail.com'):
    variants = ('.ru', '.com', '.net')
    if (sender.find('@') < 0 or recipient.find('@') < 0 or
            recipient.endswith(variants) + sender.endswith(variants) < 2):
        print(f"Невозможно отправить письмо с адреса {sender} на адрес {recipient}")
    elif sender == recipient:
        print('Нельзя отправить письмо самому себе!')
    elif sender == 'uneversity.help@gmail.com':
